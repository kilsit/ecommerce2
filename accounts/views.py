...
def register(request):
    if request.method == 'POST': 
        ...
        if form.is_valid():
            ...
            #     'victor',
            #     'test message.',
            #     'victor@skylineict.com',
            #      ['skylineict@gmail.com'],
            #      fail_silently=False,
            #  )
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(request, 'invalin login credentials')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Congratutions! Your account is activated')
        return redirect('login')
    else:
